...
import { concatHex, decodeFunctionResult, encodeAbiParameters, encodeDeployData, encodeFunctionData, getTypesForEIP712Domain, hashMessage, hashTypedData, keccak256, parseAbi, publicActions, stringToHex, validateTypedData } from "viem";
 ...
 */
export async function createKernelAccount(client, { plugins, entryPoint = KERNEL_ADDRESSES.ENTRYPOINT_V0_6, index = 0n, factoryAddress = KERNEL_ADDRESSES.FACTORY_ADDRESS, accountLogicAddress = KERNEL_ADDRESSES.ACCOUNT_LOGIC, deployedAccountAddress }) {
    ...
    const signHashedMessage = async (messageHash) => {
        let kernelImplAddr;
        try {
            const strgAddr = await getAction(client, getStorageAt)({
        ...
        const digest = keccak256(concatHex(["0x1901", domainSeparator, messageHash]));
        return kernelPluginManager.signMessage({
            message: {
                raw: digest
            }
