...
import { encodeAbiParameters, encodeFunctionData, parseAbiParameters, keccak256, publicActions, createPublicClient, http, decodeAbiParameters, } from "viem";
...
export class RecoveryProvider extends ValidatorProvider {
    ...
    static async init(params) {
        ...
        if (params.recoveryId) {
            ...
            const publicClient = createPublicClient({
                transport: http(params.opts.providerConfig?.rpcUrl ??
                    CHAIN_ID_TO_NODE[chain?.id ?? polygonMumbai.id]),
                chain: chain ?? polygonMumbai,
            });
            recoveryConfig = await RecoveryValidator.fetchRecoveryConfigFromContract(params.opts?.accountConfig?.accountAddress, publicClient);
        }
    ...
    async encodeCalldataAndNonce(enableData) {
        ...
        const encodedCallDataAndNonce = encodeAbiParameters(parseAbiParameters("address sender, bytes calldata, uint256 nonce"), [sender, callData, nonce]);
    ...
    async renewRecovery() {
        ...
        const [guardians, weights, threshold, delay] = decodeAbiParameters(parseAbiParameters("address[] guardians, uint24[] weights, uint24 threshold, uint48 delay"), enableData);
    ...
    async initiateRecovery(enableData) {
        ...
        const recoveryId = await postRecoveryData(enableData, await this.getAddress(), serializedConfig);
