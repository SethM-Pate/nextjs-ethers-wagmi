...
class TurnkeyWalletConnectorBase extends walletConnectorCore.WalletConnectorBase {
    ...
    getWebAuthnAttestation() {
        return _tslib.__awaiter(this, void 0, void 0, function* () {
            ...
            if (!email && !phone) {
                throw new Error('Email or phone must be set to register a webauthn credential.');
            }
            const webAuthnCreateParams = {
                publicKey: {
                    ...
                    user: {
                        displayName: email || `${this.appName} - ${phone}`,
                        id: authenticatorUserId,
                        name: email || `${this.appName} - ${phone}`,
                    },
                },
            };
            let attestation;
            try {
                ...
                walletConnectorCore.logger.warn(`Unable to register webauthn credential on the current page's TLD ${utils.getTLD()}. Falling back to using hostname. ${window.location.hostname}`, error);
