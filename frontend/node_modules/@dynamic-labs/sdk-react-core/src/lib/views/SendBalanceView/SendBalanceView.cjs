...
const SendBalanceView = ({ initialRecipientAddress = '', initialValue, onClickBack, onClickClose, onError, onSuccess, onDone, displayPoweredByDynamicFooter = false, }) => {
    ...
    const buildTransactionStage = () => {
        ...
        return (jsxRuntime.jsx(TransactionStage.TransactionStage, { provider: provider, transaction: transaction, copykey: 'dyn_send_transaction.confirmation.title', title: t('dyn_send_transaction.confirmation.title'), onClickBack: () => setStage('form'), onError: onError, displayPoweredByDynamicFooter: displayPoweredByDynamicFooter, mutation: (_a) => _tslib.__awaiter(void 0, [_a], void 0, function* ({ maxFeePerGas, maxPriorityFeePerGas }) {
                walletUiUtils.disabledConfirmationOnce();
                const signer = yield walletConnector.getWalletClient();
