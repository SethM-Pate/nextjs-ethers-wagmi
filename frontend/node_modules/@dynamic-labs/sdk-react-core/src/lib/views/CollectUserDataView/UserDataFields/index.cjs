...
const UserDataFields = ({ errors, policiesConsentInnerComponentArray, fields, touched, }) => {
    ...
    return (jsxRuntime.jsx("div", { className: 'user-data-fields__fields-column', children: fields === null || fields === void 0 ? void 0 : fields.map(({ name, required, enabled }) => {
            const { label, type, key, autoComplete } = index.getValueByKey(fieldsConfig, name);
            if (name === 'tShirtSize') {
                ...
                return (jsxRuntime.jsx(Tooltip.Tooltip, { content: t('dyn_collect_user_data.update_email_tooltip'), className: 'user-data-fields__tooltip', copykey: 'dyn_collect_user_data.update_email_tooltip', children: jsxRuntime.jsx(formik.Field, { copykey: key, disabled: true, as: Input.Input, type: type, id: name, name: name, label: label, autoComplete: autoComplete }, name) }, 'disabled-email-tooltip'));
            }
            else {
                return (jsxRuntime.jsx(formik.Field, { copykey: key, disabled: !enabled, as: Input.Input, type: type, id: name, name: name, label: label, optional: enabled && !required, error: errors[name] && touched[name], message: touched[name] && getDisplayErrorMessage.getDisplayErrorMessage(label, errors[name]), autoComplete: autoComplete }, name));
            }
        }) }));
};
