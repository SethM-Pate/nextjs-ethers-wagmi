...
const TransactionConfirmationView = ({ transaction, mutation, onError, onSuccess, onClickBack, onClickClose, title, displayPoweredByDynamicFooter = false, provider, copykey, }) => {
    ...
    const { isLoading: isGasSponsoredLoading, data: canSponsorTransactionGas } = usePromise.usePromise(() => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        if (!walletConnectorCore.isAccountAbstractionConnector(primaryWallet.connector)) {
            return false;
        }
        const canSponsor = yield primaryWallet.connector.canSponsorTransactionGas(transaction);
        return canSponsor !== null && canSponsor !== void 0 ? canSponsor : false;
    }), {
        onReject: (error) => {
    ...
    const isTransactionLoading = isTransactionValueLoading ||
        (canSponsorTransactionGas ? false : isGasPriceLoading) ||
