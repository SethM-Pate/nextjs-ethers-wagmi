...
const useConnectAndSignSplitSteps = () => {
    ...
    const { connectWallet, consumeNonce, displaySiweStatement, projectSettings, environmentId, networkValidationMode, setIsSingleWalletAccount, setShowAuthFlow, user, authMode, setIsVerificationInProgress, } = useInternalDynamicContext.useInternalDynamicContext();
    ...
    // or in Dapper
    const onlyConnectUser = (_a) => _tslib.__awaiter(void 0, [_a], void 0, function* ({ walletConnector, fetchPublicAddressOpts = undefined, }) {
        var _b;
        // Raise verification ongoing flag
        setIsVerificationInProgress(true);
        yield showPendingConnectView.showPendingConnectView(walletConnector, setView);
        try {
            yield connectWallet(walletConnector, fetchPublicAddressOpts, {
                raiseOnConnect: true,
            });
    ...
    const signAlreadyConnectedUser = (_c) => _tslib.__awaiter(void 0, [_c], void 0, function* ({ walletConnector, fetchPublicAddressOpts = undefined, }) {
        // Raise verification ongoing flag
        setIsVerificationInProgress(true);
        try {
