...
const useValidateSession = ({ handleLogOut, user, authToken, walletConnectV1Bridge, walletConnectors, wcVersion, }) => {
    ...
    const validate = () => _tslib.__awaiter(void 0, void 0, void 0, function* () {
        yield handleUserDataOutOfSync();
        yield handleConnectorBridgeOutOfSync();
        yield handleConnectorVersionOutOfSync();
    });
    if (!hasLoggedOut) {
        validate();
    }
    return { setHasLoggedOut };
};
